...
class App extends Component {
  ...
  slotChanged(tid, sid, newVal) {
    const tracks = this.state.tracks;
    const index = tracks[tid].slots.findIndex(el => el.id === sid);
  ...
  render() {
    ...
    return (
      <div className="App">
        <aside>
          <div onClick={this.showSettings.bind(this)}>
            <i className="fas fa-cogs" />
          </div>
          {this.state.showSettings ? <Settings /> : ''}
          <h3>Tracks</h3>
          <button onClick={el => this.showTrackForm(el)}>Add Track</button>
          {this.state.showTrackForm ? (
            <AddTrackForm onTrackAdded={el => this.trackAdded(el)} />
          ) : (
            ''
          )}
        </aside>
        <section className="weeks">{weeks}</section>
        <aside className="trackNames">{trackNames}</aside>
        <section className="tracks">{tracks}</section>
        <h4>Notes</h4>
        <ol>
